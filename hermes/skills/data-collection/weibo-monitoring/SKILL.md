...
**链内链接/评论配图型（2026-09-19 实测，实例 5344846044990977）**：引用链段落内的链接：`text` 显示为链接文案（如「评论配图」）、`raw_text` 给出实际 URL（t.cn 短链）。交付：正文保留显示文案、📎 注 注明实际指向＋可点击 URL（t.cn 实测解析 302 → `photo.weibo.com/h5/comment/compic_id/<id>` 评论配图页，可达）。

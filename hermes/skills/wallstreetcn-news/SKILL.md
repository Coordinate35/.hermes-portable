...
用户说"今日头条"、"头条"、"新闻"、"热点新闻"、"播报新闻"等时，**指的是华尔街见闻头条**，不是今日头条（toutiao.com）平台。这是用户的投资分析语境下的默认新闻来源。

- ✅ "今日头条" → 华尔街见闻 API / `parse-headlines.py`
- ❌ "今日头条" → `browser_navigate("https://www.toutiao.com")`

原因：用户关注财经投资，华尔街见闻是其首选财经新闻源。今日头条是通用新闻平台，内容杂且与投资分析无关。

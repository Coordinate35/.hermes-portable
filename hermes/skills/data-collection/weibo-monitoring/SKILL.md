...
- **交付前逐字节核验（2026-09-23 实测抓到真实笔误）**：拟交付的「📄 完整原文」块必须与 show 端点 `data.text`（去 HTML）脚本化逐字节 diff，0 差异才发。提取块：`re.search(r'📄 完整原文\n(.+?)\n\n📎 注', 拟发稿, re.S)`；实测抓出「寥寥。」被误抄为「寥寥，」类笔误——LLM 抄写长文必有出错概率，本法零成本兜底（.py 落盘执行，PYTHONPATH 行内前缀）。

...
**补充（2026-09 实证）**：文字与**非音频**媒体（图片/PDF）混排是**安全的**——实现上文字先发为一条消息，图片/文件随后作为独立消息投递（extract_media 拆分：文字→send、图片→send_multiple_images、文件→send_document）。实证：9-16 潘功胜文章"文字说明+MEDIA:PDF"成功送达。此补充仅限非音频；**音频仍必须独占整条回复**。
